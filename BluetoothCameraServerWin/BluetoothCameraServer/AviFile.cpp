...
HRESULT CAviFile::InitMovieCreation(int nFrameWidth, int nFrameHeight, int nBitsPerPixel)
{
	...
	if(m_hAviDC==NULL)	
	{
		SetErrorMessage(L"Unable to Create Compatible DC");
		return E_FAIL;
	}
	...
	if(m_hHeap==NULL)
	{
		SetErrorMessage(L"Unable to Create Heap");
		return E_FAIL;
	}
	...
	if(m_lpBits==NULL)	
	{	
		SetErrorMessage(L"Unable to Allocate Memory on Heap");
		return E_FAIL;
	}

	if(FAILED(AVIFileOpen(&m_pAviFile, m_szFileName, OF_CREATE|OF_WRITE, NULL)))
	{
		SetErrorMessage(L"Unable to Create the Movie File");
		return E_FAIL;
	}
	...
	{
		SetErrorMessage(L"Unable to Create Video Stream in the Movie File");
		return E_FAIL;
	}
	...
	if(FAILED(AVIMakeCompressedStream(&m_pAviCompressedStream,m_pAviStream,&m_AviCompressOptions,NULL)))
	{
		...
		SetErrorMessage(L"Unable to Create Compressed Stream: Check your CODEC options");
		return E_FAIL;
	}
	...
	if(FAILED(AVIStreamSetFormat(m_pAviCompressedStream,0,(LPVOID)&bmpInfo, bmpInfo.bmiHeader.biSize)))
	{
		...
		SetErrorMessage(L"Unable to Set Video Stream Format");
		return E_FAIL;
	}
